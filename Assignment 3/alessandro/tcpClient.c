...
void sendByeMessage(){
  ...
  send(service.serverFD, message, strlen(message), 0);
...
void sendProbeMessages(){
  ...
  if(strcmp(service.measureType, "rtt") == 0){
    ...
    printf("Numero di byte: %f\n", numberOfBits/8);
    printf("Numero di kilobits: %f\n", numberOfBits/1000);
    printf("Rtt medio in secondi: %f\n", (evaluateRTT(rttOfProbes)/1000000));
